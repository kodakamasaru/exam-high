...
- ただし、以下のような拡張が将来的に行われることを想定して設計すること
  - リソース種別の追加（会議室以外に機材・車両など）
  - 予約時の通知（メール、Slack、Webhook等）
  - 外部カレンダー連携（Google Calendar等）
  - フロントエンド以外からのAPI利用（モバイルアプリ、外部システム連携）
  - 承認フローや繰り返し予約などの業務ルール追加
...
本試験ではAIツール（GitHub Copilot、ChatGPT、Claude、Cursor等）の利用を制限しません。
すべて手でやると大きく時間がかかる分量となっているので積極的に利用してください。
